...
static class StandardSecurityHandler
{
    ...
    public static Task<byte[]?> HandleSecurityExchangeAsync(RdpSession session, byte[] packet)
    {
        ...
        {
            session.ConsoleLog(ConsoleLogLevel.Error,
                $"  [!] Security exchange parse error: {ex.Message}");
    ...
    public static byte[]? HandleInfoPDU(RdpSession session, byte[] packet)
    {
        ...
        try
        {
            ...
            if (payload == null || payload.Length == 0)
            {
                session.ConsoleLog(ConsoleLogLevel.Error, "  [!] Info PDU: 無法提取 payload");
                state.Phase = SessionPhase.Error;
                return null;
            }
            ...
            if (cred != null)
                ...
                session.ConsoleLog(ConsoleLogLevel.Error,
                    $"  [!] Info PDU: 解析失敗 (payload={payload.Length} bytes, hasKey={state.DecryptKey != null})");
        }
        catch (Exception ex)
        {
            session.ConsoleLog(ConsoleLogLevel.Error,
                $"  [!] Info PDU parse error: {ex.Message}");
        }
    ...
    static void LogInfoPdu(RdpSession session, byte[] payload)
    {
        ...
        try
        {
            ...
            {
                state.Credential = cred with { ClientInfo = state.ClientInfo };
                if (session.IsConsoleLevelEnabled(ConsoleLogLevel.Credential))
                {
            ...
            {
                session.ConsoleLog(ConsoleLogLevel.Error,
                    $"  [!] Info PDU parse failed (payload={payload.Length} bytes)");
            }
        }
        catch (Exception ex)
        {
